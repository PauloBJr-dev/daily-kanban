...
export const StudioSidebar: React.FC<StudioSidebarProps> = ({
  ...
  if (!isOpen) {
    return (
      <aside
        ...
        className="hidden md:flex w-14 shrink-0 border-r border-slate-200/80 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-900/70 flex-col items-center py-4 gap-3 transition-all"
      >
        <button
          type="button"
          onClick={onToggle}
  ...
  return (
    <>
      {/* Mobile Backdrop */}
      <div
        className="md:hidden fixed inset-0 z-30 bg-slate-950/50 backdrop-blur-xs transition-opacity"
        onClick={onToggle}
        aria-hidden="true"
      />
      ...
      <aside
        ...
        className="fixed inset-y-0 left-0 z-40 w-80 max-w-[85vw] shadow-2xl bg-white dark:bg-slate-900 border-r border-slate-200/80 dark:border-slate-800 md:relative md:inset-auto md:z-auto md:w-72 sm:md:w-80 md:shadow-none flex flex-col h-full overflow-hidden select-none"
      >
        {/* Header */}
